...
const WheelPickerItem: React.FC<ItemProps> = ({
  ...
  const scale = relativeScrollIndex.interpolate({
    ...
    outputRange: (() => {
      ...
      for (let x = 1; x <= visibleRest + 1; x++) {
        ...
        range.unshift(y - tempValue);
        range.push(y - tempValue);
        tempValue += 0.1;
      }
      return range;
    })(),
  ...
  // const fontWeight = fontWeightAnimation.interpolate({
  //   inputRange: (() => {
  //     const range = [-2, -1, 0, 1, 2];
  //     return range;
  //   })(),
  //   outputRange: (() => {
  //     const range = ['400', '500', '700', '500', '400'];
  //     // const range = [50, 24, 30, 24, 50];
  //     return range;
  //   })(),
  // });
  ...
  //     fontSizeAnimation.value,
  //     [0, 1],
  //     [13, 20],
  //   );

  //   return {fontSize: interpolation};
  // });
  ...
  return (
    <Animated.View
      ...
      <Animated.Text numberOfLines={1} style={[styles.itemTxt, textStyle]}>
        {option}
      </Animated.Text>
    </Animated.View>
  );
...
const styles = StyleSheet.create({
  ...
  itemTxt: {maxWidth: '85%', textAlign: 'center'},
});
