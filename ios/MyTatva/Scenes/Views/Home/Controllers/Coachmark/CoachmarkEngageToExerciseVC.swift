...
class CoachmarkEngageToExerciseVC: ClearNavigationFontBlackBaseVC {
    ...
    private func setUpView() {
        ...
        self.lblTitle.text = CoachmarkListModel.shared.description(page: .engage_exercise)
        ...
        DispatchQueue.main.async {
            ...
            Settings().isHidden(setting: .hide_engage_page) { isHidden in
                if isHidden {
                    self.showcase.setTargetView(tabBar: self.targetTabbar, itemIndex: 4, tapThrough: false)
                }
                else {
                    self.showcase.setTargetView(tabBar: self.targetTabbar, itemIndex: 5, tapThrough: false)
                }
            }
