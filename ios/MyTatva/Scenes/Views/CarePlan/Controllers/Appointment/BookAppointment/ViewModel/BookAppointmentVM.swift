    ...
    private func isValidView(clinic_id: String,
        ...
        if forCoach {
            if health_coach_id.trim() == "" {
                return AppError.validation(type: .PleaseSelectCoach)
            }
        }
        else {
            if clinic_id.trim() == "" {
                ...
                if self.isNextPage {
                    ...
                    if self.arrList.count > 0 {
                        ...
                        self.appointment_time_slotAPI(clinic_id: clinic_id,
                                                      ...
                                                      colView: colView,
                                                      withLoader: false,
                                                      forCoach: forCoach) { [weak self] (isDone) in
...
extension BookAppointmentVM {
    ...
    func apiCallSubmit(vc: UIViewController,
        ...
        params2[AnalyticsParameters.type.rawValue]   = params["type"]
        FIRAnalytics.FIRLogEvent(eventName: .USER_CONFIRM_BOOK_APPOINTMENT,
        ...
        ApiManager.shared.makeRequest(method: ApiEndPoints.doctor(.add_appointment), methodType: .post, parameter: params, withErrorAlert: true, withLoader: true, withdebugLog: true) { (result) in
            ...
            case .success(let response):
                ...
                case .success:
                    ...
                    self.strUrl = response.data["url"].stringValue
...
//                    }
    ...
    func fetch_videocall_dataAPI(withLoader: Bool,
                                 completion: ((Bool) -> Void)?){
        ...
        ApiManager.shared.makeRequest(method: ApiEndPoints.doctor(.fetch_videocall_data), methodType: .post, parameter: params, withErrorAlert: true, withLoader: withLoader, withdebugLog: true) { (result) in
