...
class LoginOTP2VC: ClearNavigationFontBlackBaseVC {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        WebengageManager.shared.navigateScreenEvent(screen: .LoginOtp)
    }
    ...
    @IBAction func btnResendTapped(_ sender: Any) {
        ...
        GlobalAPI.shared.sendOtpAPI(country_code: "+91",
                                    ...
                                    type: .login,
                                    screen: .LoginOtp) { [weak self]  (isDone, userAlreadyRegistered) in
...
extension LoginOTP2VC: VPMOTPViewDelegate {
...
extension LoginOTP2VC {
    private func setupViewModelObserver() {
        ...
        self.viewModel.otpResult.bind(observer: { [weak self]  (result) in
            guard let self = self else {return}
            switch result {
            case .success(_):
