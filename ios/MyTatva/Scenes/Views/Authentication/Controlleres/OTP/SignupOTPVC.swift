...
class SignupOTPVC: ClearNavigationFontBlackBaseVC {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        WebengageManager.shared.navigateScreenEvent(screen: .SignUpOtp)
    }
    ...
    private func applyStyle() {
        self.lblVerifyMobile.font(name: .bold, size: 22).textColor(color: .themePurple)
    ...
    @IBAction func btnResendTapped(_ sender: Any) {
        GlobalAPI.shared.sendOtpAPI(country_code: "+91",
                                    ...
                                    type: .signup,
                                    screen: .SignUpOtp) { [weak self] (isDone, userAlreadyRegistered) in
...
extension SignupOTPVC: VPMOTPViewDelegate {
    ...
    private func setupViewModelObserver() {
        ...
        self.viewModel.otpResult.bind(observer: { (result) in
            ...
            case .success(_):
                ...
                var obj                             = JSON()
                obj["isDone"]                       = "Yes"
                self.verifyUserModel                = self.viewModel.verifyUserModel
