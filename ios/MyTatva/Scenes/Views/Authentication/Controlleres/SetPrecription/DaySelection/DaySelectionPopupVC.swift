...
class DaySelectionPopupVC: ClearNavigationFontBlackBaseVC {
    ...
    //MARK:- Class Variable
    let viewModel                       = DaySelectionPopupVM()
    let refreshControl                  = UIRefreshControl()
    var strErrorMessage : String        = ""
    
    var completionHandler: ((_ obj : [DaysListModel]?) -> Void)?
    var isFromPrescription: Bool = false
    var arrDaysOffline : [DaysListModel] = []
    var arrDays : [JSON] = [
        [
            "name" : "Sunday",
            "short" : "Sun",
    ...
    fileprivate func dismissPopUp(_ animated : Bool = true, objAtIndex : [DaysListModel]? = nil) {
...
//            }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.viewModel.isFromPrescription = self.isFromPrescription
        self.viewModel.apiCallFromStart(refreshControl: self.refreshControl,
                                        tblView: self.tblView,
                                        withLoader: true)
    }
            ...
            if self.viewModel.arrList.count > 0 {
                for parent in self.arrDaysOffline {
                    for child in self.viewModel.arrList {
                        ...
                        if parent.day.lowercased() == child.day.lowercased() {
...
//MARK:- UITableView Methods
//----------------------------------------------------------------------------
extension DaySelectionPopupVC : UITableViewDataSource, UITableViewDelegate{
...
extension DaySelectionPopupVC : DZNEmptyDataSetDelegate, DZNEmptyDataSetSource {
