...
//MARK: ---------------- Update Data ----------------------
extension AddGoalsVM {
    
    func getCount() -> Int {
        return self.arrList.count
    }
    
    func getObject(index: Int) -> GoalListModel {
    ...
    func manageSelection(index: Int) {
        ...
        for item in self.arrList {
            ...
            if item.goalMasterId == object.goalMasterId {
                if item.isSelected {
    ...
    func getSelectedObject() ->  [GoalListModel] {
        var  arrTemp = [GoalListModel]()
        for item in self.arrList {
            if item.isSelected {
                arrTemp.append(item)
            }
...
extension AddGoalsVM {
    ...
    func goalListAPI(tblView: UITableView? = nil,
                             withLoader: Bool,
...
//        })
        ...
        ApiManager.shared.makeRequest(method: ApiEndPoints.patient(.goal_list), methodType: .post, parameter: params, withErrorAlert: true, withLoader: withLoader, withdebugLog: true) { (result) in
