//
//  AppoAppointmentDateAndTimeVM.swift
//  MyTatva
//
...
class AppointmentDateAndTimeVM {
...
extension AppointmentDateAndTimeVM {
...
extension AppointmentDateAndTimeVM {
    ...
    func get_bcp_time_slots(date:Date,
                            withLoader: Bool,
        ...
        ApiManager.shared.makeRequest(method: ApiEndPoints.doctor(.get_bcp_time_slots), methodType: .post, parameter: params, withErrorAlert: true, withLoader: withLoader, withdebugLog: true) { (result) in
            ...
            case .success(let response):
                ...
                case .invalidOrFail:
                    ...
                    break
                case .success:
                    self.arrSlotsData.removeAll()
                    self.arrSlotsData.append(response.data.arrayValue.map({
                        TimeSlot.init(fromJson: $0)
                    }))
                    returnVal = true
