...
class SelectTestSlotListCell : UITableViewCell {
...
class SelectTestSlotVC: LightPurpleNavigationBase {
    //MARK: - Outlets -
    ...
    let viewModel                   = SelectTestSlotVM()
...
//            self.btnReviewDetails.alpha = self.selectedTimeSlot.isEmpty ? 0.5 : 1.0
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.lblNavTitle.text = self.isFromPhysio ? "With Physiotherapist" : "With Nutritionist"
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
            ...
            let currentDay = calender.date(byAdding: .day, value: 1, to: Date()) ?? Date()
            self.viewModel.get_bcp_time_slots(date: currentDay,withLoader: true) { isDone in
...
//        self.btnReviewDetails.alpha = 0.5
    ...
    private func setCalendar(){
        ...
        let calender = Calendar.current
        let currentDay = calender.date(byAdding: .day, value: 1, to: Date()) ?? Date()
        self.selectedDate = currentDay//calender.date(byAdding: .weekday,value: -3, to: currentDay) ?? Date()
        
        self.endDate = calender.date(byAdding: .weekday, value: 14, to: currentDay) ?? Date()
        
        var arrDates = Date.dates(from: currentDay, to: self.endDate)
        ...
        self.endDate = calender.date(byAdding: .weekday, value: dateCount + tempCount, to: currentDay) ?? Date()
        
        self.calendar.backgroundColor                   = .clear
        self.calendar.delegate                          = self
        self.calendar.dataSource                        = self
        self.calendar.today                             = currentDay
...
//        diyCell.selectionLayer.fillColor = UIColor.colorFromHex(hex: 0x946FB8).withAlphaComponent(0.1).cgColor
        
        diyCell.selectionType = .single
        diyCell.titleLabel.frame = diyCell.circleImageView.frame
        diyCell.titleLabel.textAlignment = .center
        
//        diyCell.selectionLayer.isHidden = false //date == self.startDate || date == self.endDate
...
//        let startTime: String = (dates.first?.trim() ?? "").replacingOccurrences(of: "AM", with: "").replacingOccurrences(of: "PM", with: "").trim()
//        let endTime: String = (dates.last?.trim() ?? "").replacingOccurrences(of: "AM", with: "").replacingOccurrences(of: "PM", with: "").trim()
                
        if isFromPhysio {
            self.viewModel.update_bcp_hc_details(patient_plan_rel_id: kPateintPlanRefID, physiotherapist_availability_date: stringDate, physiotherapist_start_time: startDate24, physiotherapist_end_time: endDate24, withLoader: true) { isDone, msg  in
                if isDone {
                    self.navigationController?.popViewController(animated: true)
                    DispatchQueue.main.async {
                        Alert.shared.showSnackBar(msg, isBCP: true)
                    }
                }
            }
        } else {
            self.viewModel.update_bcp_hc_details(patient_plan_rel_id: kPateintPlanRefID, nutritionist_availability_date: stringDate, nutritionist_start_time: startDate24, nutritionist_end_time: endDate24, withLoader: true) { isDone, msg  in
                if isDone {
                    self.navigationController?.popViewController(animated: true)
                    DispatchQueue.main.async {
                        Alert.shared.showSnackBar(msg, isBCP: true)
...
extension SelectTestSlotVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withClass: SelectTestSlotListCell.self)
...
extension SelectTestSlotListCell : UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
...
extension SelectTestSlotVC : HorizontalCalendarDelegate {
        ...
        if Calendar.current.compare(date, to: Date(), toGranularity: .day) == .orderedAscending {
            ...
            self.pendingRequest = DispatchWorkItem{ [weak self] in
                ...
                if formatter.string(from: self.selectedDate) == "Sun" {
                    self.lblNoData.text = "Looks like, there are no slots available, please check for a different date to get the slots"
                    self.lblNoData.isHidden = false
                    self.tblTimeSlot.isHidden = true
                    self.lblChooseTime.isHidden = true
                }else {
...
extension SelectTestSlotVC : UITextFieldDelegate {
...
extension SelectTestSlotListCell {
...
extension SelectTestSlotVC: FSCalendarDelegate, FSCalendarDataSource, FSCalendarDelegateAppearance {
    
    func calendar(_ calendar: FSCalendar, imageFor date: Date) -> UIImage? {
        if date == self.selectedDate {
            return UIImage(named: "AC")
        }
        return nil
    }
    ...
    func calendar(_ calendar: FSCalendar, cellFor date: Date, at position: FSCalendarMonthPosition) -> FSCalendarCell {
        ...
        cell.circleImageView.frame = CGRect(x: cell.contentView.frame.width / 2, y: cell.contentView.frame.height / 2, width: 40, height: 32)
        return cell
    ...
    func calendar(_ calendar: FSCalendar, boundingRectWillChange bounds: CGRect, animated: Bool) {
        ...
        self.calendarHeight.constant = bounds.height //+ 25
    }
    
    func maximumDate(for calendar: FSCalendar) -> Date {
        return self.endDate //Calendar.current.date(byAdding: .year, value: 5, to: Date()) ?? Date()
    }
...
//        guard self.selectedDate != date else { return }
    ...
    func calendar(_ calendar: FSCalendar, appearance: FSCalendarAppearance, titleDefaultColorFor date: Date) -> UIColor? {
        return (date <= Date() || date > self.endDate) ? UIColor.colorFromHex(hex: 0x9E9E9E) : .themeGray5
    }
...
//    weak var selectionLayer: CAShapeLayer!
    ...
    var selectionType: FSSelectionType = .none {
        didSet {
            setNeedsLayout()
        }
    }
...
////        self.selectionLayer = selectionLayer
    ...
    override func layoutSubviews() {
        super.layoutSubviews()
        self.circleImageView.frame = CGRect(x: self.contentView.frame.origin.x, y: self.contentView.frame.origin.y, width: 40, height: 32)
        self.circleImageView.backGroundColor(color: .themePurple).cornerRadius(cornerRadius: 12)
        self.circleImageView.contentMode = .center
        self.backgroundView?.frame = self.bounds.insetBy(dx: 1, dy: 1)
        /*self.selectionLayer.frame = CGRect(x: self.contentView.bounds.minX, y: self.contentView.bounds.minY, width: 0, height: 0)
        
        let x = self.selectionLayer.bounds.minX
        let y = self.selectionLayer.bounds.minY*/
        ...
        /*if selectionType == .middle {
            ...
            if code == "ar"{
                self.selectionLayer.path = UIBezierPath(roundedRect: self.selectionLayer.bounds, byRoundingCorners: [.topRight, .bottomRight], cornerRadii: CGSize(width: 16.0, height: 16.0)).cgPath
            }else{
                
                self.selectionLayer.path = UIBezierPath(roundedRect: CGRect(x: x + 10, y: y, width: self.selectionLayer.bounds.width - 10, height: self.selectionLayer.bounds.height), byRoundingCorners: [.topLeft, .bottomLeft], cornerRadii: CGSize(width: 16.0, height: 16.0)).cgPath
            }
        }
        else if selectionType == .rightBorder {
            if code == "ar"{
                self.selectionLayer.path = UIBezierPath(roundedRect: self.selectionLayer.bounds, byRoundingCorners: [.topLeft, .bottomLeft], cornerRadii: CGSize(width: 6.0, height: 6.0)).cgPath
            } else {
                self.selectionLayer.path = UIBezierPath(roundedRect: CGRect(x: x, y: y, width: self.selectionLayer.bounds.width - 10, height: self.selectionLayer.bounds.height), byRoundingCorners: [.topRight, .bottomRight], cornerRadii: CGSize(width: 16.0, height: 16.0)).cgPath
            }
        }else if selectionType == .single {
            self.selectionLayer.path = UIBezierPath(roundedRect: CGRect(x: x + 10.0, y: y, width: 0, height: self.selectionLayer.bounds.height), byRoundingCorners: [.topRight, .bottomRight , .topLeft , .bottomLeft], cornerRadii: CGSize(width: 16.0, height: 16.0)).cgPath
        }*/
    }
