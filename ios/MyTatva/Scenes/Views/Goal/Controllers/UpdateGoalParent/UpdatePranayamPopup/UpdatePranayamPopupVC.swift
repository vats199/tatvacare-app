...
class UpdatePranayamPopupVC: ClearNavigationFontBlackBaseVC {
    ...
    @IBOutlet weak var vwPranayamVideo      : UIView!
    @IBOutlet weak var lblPranayamVideo     : UILabel!
    
    @IBOutlet weak var lblPranayamDuration  : UILabel!
    @IBOutlet weak var lblPranayamDurationValue  : UILabel!
...
//    var timeFormat                      = DateTimeFormaterEnum.hhmma.rawValue
    ...
    fileprivate func setUpView() {
        ...
        self.lblPranayamVideo.font(name: .medium, size: 14)
            .textColor(color: UIColor.themeBlack)
        
        self.lblPranayamDuration.font(name: .regular, size: 14)
            .textColor(color: UIColor.themeBlack)
        self.lblPranayamDurationValue.font(name: .medium, size: 14)
            .textColor(color: UIColor.themeBlack)
        ...
        DispatchQueue.main.async {
            ...
            self.vwPranayamVideo.layoutIfNeeded()
            ...
            self.vwPranayamVideo.cornerRadius(cornerRadius: 5)
    ...
    fileprivate func manageActionMethods(){
        ...
        self.btnAdd.addTapGestureRecognizer {
            ...
            self.viewModel.apiCall(vc: self,
                                   date: self.txtDate,
                                   time: self.txtTime,
                                   pranayam_duration: self.lblPranayamDurationValue.text!,
        ...
        self.vwPranayamVideo.addTapGestureRecognizer {
            let obj         = JSON()
            self.dismissPopUp(true, objAtIndex: obj)
            
            let vc = ExerciseStartVC.instantiate(fromAppStoryboard: .exercise)
            let goal                = Int(self.goalListModel.goalValue) ?? 0
            let todayAchieve        = Int(self.goalListModel.todaysAchievedValue) ?? 0
            vc.goalMasterId         = self.goalListModel.goalMasterId
            vc.maxTime              = (goal) * 60
            vc.timerTime            = (goal - todayAchieve) * 60
            vc.achievedTime         = (goal - todayAchieve) * 60
...
extension UpdatePranayamPopupVC {
...
extension UpdatePranayamPopupVC : UITextFieldDelegate {
...
extension UpdatePranayamPopupVC {
    ...
    fileprivate func handleLog(sender: UIButton){
        switch sender {
        case self.btnPlus:
            if self.valueLog < kMaximumPranayamLog {
                self.valueLog += kJumpValuePranayam
            }
            break
        case self.btnMinus:
            if self.valueLog > kJumpValuePranayam {
                self.valueLog -= kJumpValuePranayam
            }
            break
        default:break
    ...
    fileprivate func calculateLogValue(){
        self.lblPranayamDurationValue.text = String(format: "%.f", Float(self.valueLog))
    }
}
...
extension UpdatePranayamPopupVC {
    ...
    fileprivate func setData(){
        ...
        GFunction.shared.setGoalProgressData(goalListModel: self.goalListModel,
                                             ...
                                             lblDuration: self.lblPranayamDurationValue, valueLog: Float(self.valueLog))
