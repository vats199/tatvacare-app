...
class UpdateBpReadingPopupVC: ClearNavigationFontBlackBaseVC {
    ...
    @IBOutlet weak var txtUnitDiastolic : UITextField!
    
    @IBOutlet weak var lblSystolic      : UILabel!
    @IBOutlet weak var txtSystolic      : UITextField!
    @IBOutlet weak var txtUnitSystolic  : UITextField!
    ...
    //MARK:- Class Variable
    let viewModel                       = UpdateBpReadingPopupVM()
...
//    var timeFormat                      = DateTimeFormaterEnum.hhmma.rawValue
    ...
    fileprivate func setUpView() {
        ...
        self.lblTime.font(name: .regular, size: 14)
            .textColor(color: UIColor.themeBlack)
        self.lblDiastolic.font(name: .regular, size: 14)
            .textColor(color: UIColor.themeBlack)
        self.lblSystolic.font(name: .regular, size: 14)
            .textColor(color: UIColor.themeBlack)
        ...
        self.txtUnitDiastolic.textAlignment = .center
        self.txtUnitDiastolic.isUserInteractionEnabled = false
        
        
        self.txtUnitSystolic
            .font(name: .medium, size: 14)
            .textColor(color: UIColor.themeBlack)
            .borderColor(color: UIColor.ThemeBorder, borderWidth: 1)
            .cornerRadius(cornerRadius: 5)
            .backGroundColor(color: UIColor.themeLightGray)
        self.txtUnitSystolic.textAlignment = .center
        self.txtUnitSystolic.isUserInteractionEnabled = false
        
        self.txtDiastolic.maxLength         = kMaxBPDiastolic.size
        self.txtSystolic.maxLength          = kMaxBPSystolic.size
        
        self.txtDiastolic.keyboardType      = .numberPad
        self.txtDiastolic.regex             = Validations.RegexType.OnlyNumber.rawValue
    ...
    fileprivate func manageActionMethods(){
        ...
        self.btnSubmit.addTapGestureRecognizer {
            self.viewModel.apiCall(vc: self,
                                   date: self.txtDate,
                                   time: self.txtTime,
                                   txtDiastolic: self.txtDiastolic,
                                   txtSystolic: self.txtSystolic,
...
extension UpdateBpReadingPopupVC {
...
extension UpdateBpReadingPopupVC : UITextFieldDelegate {
...
extension UpdateBpReadingPopupVC {
    ...
    fileprivate func setData(){
        ...
        self.txtSystolic.text       = self.readingListModel.readingValueData.systolic ?? "0"
        self.txtDiastolic.text      = self.readingListModel.readingValueData.diastolic ?? "0"
        
        self.txtUnitDiastolic.text  = self.readingListModel.measurements
        self.txtUnitSystolic.text   = self.readingListModel.measurements
...
//                                                                   status: .NOCONVERSION)
                ...
                HealthKitManager.shared.addLogToHealthKit(identifier: .bloodPressureSystolic,
                                                          reading: Double(self.txtSystolic.text!) ?? 0,
                ...
                HealthKitManager.shared.addLogToHealthKit(identifier: .bloodPressureDiastolic,
                                                          reading: Double(self.txtDiastolic.text!) ?? 0,
                                                          unit: HKUnit.millimeterOfMercury(),
                                                          dateTime: self.viewModel.achieved_datetime)
                if self.isNext {
