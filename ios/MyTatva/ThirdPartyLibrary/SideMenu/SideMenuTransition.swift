...
extension SideMenuTransition: UIViewControllerAnimatedTransitioning {
    ...
    open func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        DispatchQueue.main.async {
            //let img = UIImageView.init(frame: container.frame)
            //img.image = UIImage(named: "MenuMask")
        ...
        let vwCorner = topView.subviews[0].subviews[0].subviews[0]
        
        DispatchQueue.main.async {
           
//            let shadowLayer             = CAShapeLayer()
//            shadowLayer.path            = UIBezierPath(roundedRect: topView.frame, cornerRadius: 20).cgPath
//            shadowLayer.fillColor       = UIColor.white.cgColor
//
//            shadowLayer.shadowColor     = UIColor.themeBlack.cgColor
...
//            topView.layer.insertSublayer(shadowLayer, at: 0)
            ...
            topView.applyViewShadow(shadowOffset: CGSize(width: 1, height: 1), shadowColor: UIColor.themeBlack, shadowOpacity: 0.4, shdowRadious: 7)
        ...
        let animate = {
            if self.presenting {
                ...
                vwCorner.cornerRadius(cornerRadius: 20, clips: true)
            } else {
                self.hideMenuStart()
                vwCorner.cornerRadius(cornerRadius: 0, clips: true)
            }
        }
