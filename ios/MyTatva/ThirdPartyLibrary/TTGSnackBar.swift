...
//        if let superView = containerView ?? UIApplication.shared.windows.first { // UIApplication.shared.keyWindow
        if let superView = containerView ?? sceneDelegate.window ?? UIApplication.shared.windows.first ?? UIApplication.shared.delegate?.window ?? sceneDelegate.window {
...
public extension TTGSnackbar {
    ...
    fileprivate func dismissAnimated(_ animated: Bool) {
        ...
        if animationType == .slideFromBottomToTop {
            animationType = .slideFromTopToBottom
        }
