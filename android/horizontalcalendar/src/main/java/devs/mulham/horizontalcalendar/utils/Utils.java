...
public final class Utils {
     ...
     * calculate each item width depends on {@link HorizontalCalendar#numberOfDatesOnScreen}
     */
    public static int calculateCellWidthAsPerCalView(Context context, int itemsOnScreen) {
        WindowManager windowManager = ((WindowManager) context.getSystemService(Context.WINDOW_SERVICE));
        if (windowManager != null) {
            Display display = windowManager.getDefaultDisplay();
            Point size = new Point();
            ...
            display.getSize(size);
            int screenWidth = size.x - ((int) context.getResources().getDimension(R.dimen.dp_32));
