        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (event != null) {
                ...
                if (Event.EventName.FCEventMessageSent == event.eventName) {
                    ...
                    if (ChatListFragment.isFirstMessageSentAPICalled.not()) {
                        //callAPI()
                    }
                }
    ...
    private fun callAPI() {
        scope.launch {
            val response = authRepository.linkHealthCoachChat(ApiRequest().apply {
                health_coach_id = ChatListFragment.healthCoachId
            })
            if (response.throwable != null) {
                Log.d(TAG, "profile:: ERROR ${response.throwable.message}")
            }
            if (response.responseBody != null) {
                ChatListFragment.isFirstMessageSentAPICalled = true
                Log.d(TAG, "profile:: Success healthCoachId :: ${ChatListFragment.healthCoachId}")
