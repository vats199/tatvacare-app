...
class AuthLiveDataSource @Inject constructor(private val authService: AuthService) :
    ...
    override suspend fun addMedicinePrescription(apiRequest: ApiRequest): DataWrapper<Any> {
        return execute { authService.addMedicinePrescription(apiRequest) }
    }

    override suspend fun fetchMedicinePrescription(apiRequest: ApiRequest): DataWrapper<ArrayList<MyPrescriptionMainData>> {
        return execute { authService.fetchMedicinePrescription(apiRequest) }
    }
