...
import com.mytatva.patient.databinding.GoalFragmentLogPranayamBinding
...
class LogPranayamFragment :
    BaseFragment<GoalFragmentLogPranayamBinding>() {
    ...
    ): GoalFragmentLogPranayamBinding {
        return GoalFragmentLogPranayamBinding.inflate(inflater, container, attachToRoot)/*.apply {
    ...
    private fun setViewListeners() {
        binding.apply {
            ...
            layoutWatchVideo.setOnClickListener { onViewClick(it) }
        }
    }
    ...
    override fun onViewClick(view: View) {
        when (view.id) {
            R.id.layoutWatchVideo -> {
                goalReadingData?.let {
                    if (it.goal_master_id.isNullOrBlank().not()) {
                        val achievedValue = it.todays_achieved_value?.toDoubleOrNull()?.toInt() ?: 0
                        val goalValue = it.goal_value?.toDoubleOrNull()?.toInt() ?: 0
                        navigator.loadActivity(IsolatedFullActivity::class.java,
                            PlanBreathingPlayerFragment::class.java).addBundle(bundleOf(
                            Pair(Common.BundleKey.GOAL_MASTER_ID, it.goal_master_id),
                            Pair(Common.BundleKey.TODAYS_ACHIEVED_VALUE, achievedValue),
                            Pair(Common.BundleKey.GOAL_VALUE, goalValue)
