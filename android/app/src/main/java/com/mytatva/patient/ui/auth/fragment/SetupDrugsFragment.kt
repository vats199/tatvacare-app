...
class SetupDrugsFragment : BaseFragment<AuthFragmentSetupDrugsBinding>() {
    ...
    private val setUpDrugAddedPrescriptionAdapter by lazy {
        SetUpDrugAddedPrescriptionAdapter(
            addedPrescriptionList,
            /*object : SetUpDrugAddedPrescriptionAdapter.AdapterListener {
                override fun onImageClick(position: Int) {
                    addedPrescriptionList[position].imagePath.let {
                ...
                override fun onPdfClick(position: Int) {
                    addedPrescriptionList[position].imagePath.let {
                        navigator.openPdfViewer(it,addedPrescriptionList[position].isAlreadyUploaded.not())
                    }
                }
            }*/
        )
        ...
        when (view.id) {
            ...
            R.id.editTextSuggestedDosage -> {
                activity?.supportFragmentManager?.let {
                    SuggestedDosageBottomSheetDialog(dosateTimeList) { dosageTimeData: DosageTimeData ->
                        ...
                        binding.editTextSuggestedDosage.setText(
                            selectedDosageTimeData?.dose_type
                                ?: ""
                        )
            ...
            R.id.textViewAddMorePrescription -> {
                ImageAndVideoPicker.newInstance()
                    ...
                    .pickDocument(true) // set true to pick docs, default false
                    ...
                    .setResult(imagePickerResult = object :
                        ...
                        override fun onImagesSelected(list: ArrayList<String>) {
                            if (ImageAndVideoPicker.isValidFileSize(list.first(), 5)) {
                                ...
                                showMessage(getString(R.string.validation_invalid_file_size_5mb))
                            }
                        }

                        override fun onDocumentSelected(list: ArrayList<String>) {
                            if (ImageAndVideoPicker.isValidFileSize(list.first(), 5)) {
                                addedPrescriptionList.add(
                                    TempDataModel(
                                        imagePath = list.first(),
                                        isPdfDocFile = true
                                    )
                                )
                                setUpDrugAddedPrescriptionAdapter.notifyItemInserted(
    ...
    private fun getFileExtension(tempDataModel: TempDataModel): String {
        return if (tempDataModel.isPdfDocFile) ".pdf" else ".jpg"
    }
    ...
    private fun handleUploadPrescription(success: (uplodedList: List<String>) -> Unit) {
        ...
        for (i in 0 until addedPrescriptionList.size) {
            ...
            val fileName = UploadToAzureStorage.PREFIX_DRUG + createFileName + getFileExtension(
                addedPrescriptionList[i]
            )
    ...
    private fun observeLiveData() {
        ...
        authViewModel.addPrescriptionLiveData.observe(this,
            onChange = { responseBody ->
                hideLoader()
                analytics.logEvent(
                    analytics.NEW_USER_MEDICINE_ADDED,
                    screenName = AnalyticsScreenNames.SetUpDrugs
                )
                if (activity is IsolatedFullActivity) {
                    navigator.goBack()
    ...
    private fun setData(getPrescriptionDetailsResData: GetPrescriptionDetailsResData) {
        ...
        getPrescriptionDetailsResData.document_data?.forEachIndexed { index, documentResData ->
            addedPrescriptionList.add(TempDataModel().apply {
                ...
                isPdfDocFile = (documentResData.document_name ?: "").endsWith(".pdf", true)
            })
        }
