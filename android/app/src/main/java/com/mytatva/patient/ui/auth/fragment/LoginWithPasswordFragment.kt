...
class LoginWithPasswordFragment : BaseFragment<AuthFragmentLoginWithPasswordBinding>() {
    ...
    override fun bindData() {
        analytics.logEvent(analytics.LOGIN_PASSWORD_ATTEMPT, screenName = AnalyticsScreenNames.LoginWithPassword)
        setViewListeners()
        setUpUI()
    }
        ...
        when (view.id) {
            ...
            R.id.textViewForgotPassword -> {
                navigator.loadActivity(AuthActivity::class.java,
                    ForgotPasswordPhoneNumberFragment::class.java).start()
            }
            R.id.textViewSignUp -> {
    ...
    private fun observeLiveData() {
        authViewModel.loginLiveData.observe(this,
            onChange = { responseBody ->
                hideLoader()
                analytics.logEvent(analytics.LOGIN_PASSWORD_SUCCESS, screenName = AnalyticsScreenNames.LoginWithPassword)
