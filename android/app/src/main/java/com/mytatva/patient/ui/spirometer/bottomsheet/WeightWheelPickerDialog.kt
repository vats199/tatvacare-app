...
class WeightWheelPickerDialog : BaseBottomSheetDialogFragment<SpirometerBottomsheetDetailsTypeBinding>() {
    ...
    private fun setSelectedData(isFirstTime: Boolean = false) = with(binding) {
        if (selectedWeightInKg != "") {
            if (tabLayoutSelected.selectedTabPosition == 0) {//kg
                ...
                val selectedFeetIndext = wheelFeet.data.indexOf(wheelFeet.data.find { it?.equals(kgs.split(".")[0]) == true })
                val selectedDecimalIndex = wheelCM.data.indexOf(wheelCM.data.find { it?.equals(".".plus(kgs.split(".")[1])) == true })
                ...
                val selectedIndext = wheelFeet.data.indexOf(wheelFeet.data.find { it?.equals(lbs.split(".")[0]) == true })
                val selectedDecimalIndex = wheelCM.data.indexOf(wheelCM.data.find { it?.equals(".".plus(lbs.split(".")[1])) == true })
    ...
    private fun setWheelPicker() = with(binding) {
        ...
        val lp : ConstraintLayout.LayoutParams = wheelFeet.layoutParams as ConstraintLayout.LayoutParams
        lp.marginEnd = 0
    ...
    private fun selectedItem(): String {
        selectedWeightInKg = when (selectedUnit) {
            WeightUnit.KG -> {
                val kgs = binding.wheelFeet.data[binding.wheelFeet.currentItemPosition].toString().plus(binding.wheelCM.data[binding.wheelCM.currentItemPosition].toString())
                kgs
            }
