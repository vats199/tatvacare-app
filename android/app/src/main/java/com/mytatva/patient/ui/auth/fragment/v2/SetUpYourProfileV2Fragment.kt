...
class SetUpYourProfileV2Fragment : BaseFragment<AuthFragmentSetupProfileV2Binding>() {
        ...
        get() {
            return try {
                with(binding) {
                    ...
                    if (!isGender) {
                        showAppMessage(
                            getString(R.string.common_validation_select_gender),
                            AppMsgStatus.ERROR
                        )
                        throw ApplicationException(getString(R.string.common_validation_select_gender))
                    }
                    ...
                    if (isDummyAccessCodeApplied.not()
                        ...
                        showAppMessage(
                            getString(R.string.validation_verify_access_code),
                            AppMsgStatus.ERROR
                        )
                        //if code entered but not verified
                        throw ApplicationException(getString(R.string.validation_verify_access_code))
                    ...
                    if (binding.checkBoxAgreeTerms.isChecked.not()) {
                        showAppMessage(
                            getString(R.string.common_validation_agree_terms),
                            AppMsgStatus.ERROR
                        )
                        throw ApplicationException(getString(R.string.common_validation_agree_terms))
                    }
        ...
        get() {
            return try {
                with(binding) {
                    validator.submit(
                        editTextYourEmail,
                        inputLayoutYourEmail/*, inputLayoutYourEmail.isErrorEnabled*/
                    )
                        .checkEmpty()
                        .errorMessage(getString(R.string.common_validation_empty_email))
                        .checkValidEmail()
    ...
    private fun handleUiToShowHideAccessCodeToggleOptions() {
        with(binding) {
            if (groupAccessCode.isVisible) {
                ...
                analytics.logEvent(
                    analytics.DOCTOR_ACCESS_CODE_HIDDEN_BY_DEFAULT,
    ...
    private fun setUpHaveDoctorAccessCodeToggleUI() {
        TextDecorator.decorate(
            binding.textViewDontHaveAccessCode,
            "Click here if you don't have a doctor code."
        )
            .underline("here")
        ...
        TextDecorator.decorate(
            binding.textViewIHaveAccessCode,
            "If you have a doctor code, click here."
        )
            .underline("here")
            .setTextColor(R.color.link_blue, "here")
            .build()
    ...
    private fun setTermCondition() {
        TextDecorator.decorate(
            binding.textViewTermCondition,
            getString(R.string.choose_condition_label_terms_conditions)
        )
    ...
    private fun setUI() {
        if (FirebaseLink.Values.accessCode.isNullOrBlank().not()) {
            ...
            binding.textViewCheck.isEnabled =
                binding.editTextDoctorAccessCode.text.toString().isNotBlank()
    ...
    private fun setOnViewClickListener() = with(binding) {
        ...
        textViewDontHaveAccessCode.setOnClickListener {
            analytics.logEvent(
                analytics.USER_CLICK_DONT_HAVE_ACCESS_CODE,
                screenName = AnalyticsScreenNames.AddAccountDetails
            )
        ...
        textViewIHaveAccessCode.setOnClickListener {
            analytics.logEvent(
                analytics.USER_CLICK_HAVE_ACCESS_CODE,
                screenName = AnalyticsScreenNames.AddAccountDetails
            )
    ...
    private fun handleOnVerifyAccessCode(
        ...
        analytics.logEvent(
            analytics.ACCESS_CODE_VERIFY_SUCCESS,
            Bundle().apply {
                putString(
                    analytics.PARAM_DOCTOR_ACCESS_CODE,
                    binding.editTextDoctorAccessCode.text.toString().trim()
                )
            }, screenName = AnalyticsScreenNames.AddAccountDetails
        )
    ...
    override fun onViewClick(view: View) {
        when (view.id) {
            ...
            R.id.textViewCheck -> {
                analytics.logEvent(
                    analytics.USER_CLICK_CHECK_ACCESS_CODE,
                    screenName = AnalyticsScreenNames.AddAccountDetails
                )
                if (binding.editTextDoctorAccessCode.text.toString().trim().isBlank().not())
                    ...
                    showAppMessage(
                        getString(R.string.common_validation_empty_access_code),
                        AppMsgStatus.ERROR
                    )
            }
            ...
            Common.RequestCode.REQUEST_CAMERA_PERMISSION -> {
                if (PermissionUtil.verifyPermissions(grantResults)) {
                    ...
                    (requireActivity() as BaseActivity).showOpenPermissionSettingDialog(
                        arrayListOf(
                            BaseActivity.AndroidPermissions.Camera
                        )
                    )
                }
            }
    ...
    private fun observeLiveData() {
        authViewModel.registerTempPatientProfileLiveData.observe(this,
            onChange = { responseBody ->
                ...
                responseBody.data.let { response ->
                    analytics.logEvent(
                        analytics.USER_ADD_ACCOUNT_STEP_SUCCESS,
                        screenName = AnalyticsScreenNames.AddAccountDetails
                    )
        ...
        authViewModel.verifyDoctorAccessCodeLiveData.observe(this,
            ...
            onError = { throwable ->
                hideLoader()
                analytics.logEvent(
                    analytics.ACCESS_CODE_VERIFY_FAIL,
                    Bundle().apply {
                        putString(
                            analytics.PARAM_DOCTOR_ACCESS_CODE,
                            binding.editTextDoctorAccessCode.text.toString().trim()
