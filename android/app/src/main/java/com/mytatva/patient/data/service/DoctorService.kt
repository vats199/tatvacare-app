...
interface DoctorService {
    ...
    suspend fun addToCart(@Body apiRequest: ApiRequest): ResponseBody<AddToCartData>
    ...
    suspend fun getAppointmentSlots(@Body apiRequest: ApiRequest): ResponseBody<ArrayList<LabTestTimeSlotData>>
    ...
    @POST(URLFactory.Tests.CANCEL_LAB_TEST)
    suspend fun cancelLabTest(@Body apiRequest: ApiRequest): ResponseBody<Any>
    ...
    @POST(URLFactory.Tests.CATALOG_LIST)
    suspend fun catalogList(@Body apiRequest: ApiRequest): ResponseBody<ArrayList<CatalogListData>>

    @POST(URLFactory.Tests.SEARCH_CATALOG_LIST)
    suspend fun searchCatalogList(@Body apiRequest: ApiRequest): ResponseBody<ArrayList<TestPackageData>>

    @POST(URLFactory.Tests.CATALOG_LIST_ALL)
    suspend fun catalogListAll(@Body apiRequest: ApiRequest): ResponseBody<ArrayList<TestPackageData>>

    @POST(URLFactory.Tests.CATALOG_DETAILS)
    suspend fun catalogDetails(@Body apiRequest: ApiRequest): ResponseBody<TestPackageData>

    @POST(URLFactory.Tests.CONFIRMATION_ORDER)
    suspend fun confirmationOrder(@Body apiRequest: ApiRequest): ResponseBody<ConfirmationPaymentData>

    @POST(URLFactory.Tests.RESCHEDULED_LAB_TEST)
    suspend fun rescheduleLabTest(@Body apiRequest: ApiRequest): ResponseBody<Any>

    @POST(URLFactory.Tests.GET_NEARBY_EXPIRE_BCP_DATA)
    suspend fun getNearByExpireBCPData(@Body apiRequest: ApiRequest): ResponseBody<BcpTestMainData>
}