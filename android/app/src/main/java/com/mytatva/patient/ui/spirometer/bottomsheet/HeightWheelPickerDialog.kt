...
class HeightWheelPickerDialog : BaseBottomSheetDialogFragment<SpirometerBottomsheetDetailsTypeBinding>() {
    ...
    private fun setSelectedData() = with(binding) {
        if (selectedHeightInCm != "") {
            if (tabLayoutSelected.selectedTabPosition == 0) {
                ...
                val pairFeetInches = HeightWeightUtils.convertCmToFeetInches(selectedHeightInCm.replace("cm", "").trim().toDoubleOrNull()?.toInt().toString())
                val feet = pairFeetInches.first
                val inch = pairFeetInches.second

                val selectedFeetIndext = wheelFeet.data.indexOf(wheelFeet.data.find { it?.equals("$feet'") == true })
                val selectedInchIndext = wheelCM.data.indexOf(wheelCM.data.find { it?.equals("$inch''") == true })
                ...
                val index = wheelFeet.data.indexOf(wheelFeet.data.find { it?.equals("$cm cm") == true })
    ...
    private fun getHeightInCmFromFeetInchWheelPicker(): String {
        val feet = binding.wheelFeet.data[binding.wheelFeet.currentItemPosition].toString().replace("'", "")
        val inch = binding.wheelCM.data[binding.wheelCM.currentItemPosition].toString().replace("''", "")
    ...
    private fun setUpViewListeners() {
        with(binding) {
            ...
            var heightFeet = HeightWeightUtils.convertCmToFeetInches(selectedHeightInCm.toDoubleOrNull()?.toInt().toString()).first
            var heightInch = HeightWeightUtils.convertCmToFeetInches(selectedHeightInCm.toDoubleOrNull()?.toInt().toString()).second
    ...
    private fun selectedItem(): String {
        return when (selectedUnit) {
            HeightUnit.FEET_INCHES -> {
                ...
                binding.wheelFeet.data[binding.wheelFeet.currentItemPosition].toString().replace(" cm", "", true)
            }
        }
    }
}