    ...
    private fun setUserData() {
        with(binding) {
            ...
            if (session.user?.ethnicity.isNullOrBlank().not()) {
                ...
                selectedEthnicity = Ethnicity.values().first { it.ethnicityKey == session.user?.ethnicity }
//                editTextEthnicity.setText(session.user?.ethnicity ?: "")
            ...
            if (height/*heightCm*/.isNotEmpty()) {
                if (selectedHeightUnit.unitKey == HeightUnit.FEET_INCHES.unitKey) {
                    ...
                    editTextHeight.setText(String.format("%s %s", height/*heightCm*/, selectedHeightUnit.unitName))
                }
            }
    ...
    private fun showEthnicitySelection() {
        ...
        CommonWheelPickerDialog.showBottomSheet(
            ...
            binding.editTextEthnicity.setText(selectedEthnicity?.ethnicityName ?: ""/*resources.getStringArray(R.array.ethnicity).get(it)*/)
        }.show(requireActivity().supportFragmentManager, CommonWheelPickerDialog::class.java.simpleName)
    ...
    private fun showHeightUnitSelection() {
        ...
        ).show(requireActivity().supportFragmentManager, HeightWheelPickerDialog::class.java.simpleName)
    }
    ...
    private fun showWeightUnitSelection() {
        ...
        ).show(requireActivity().supportFragmentManager, WeightWheelPickerDialog::class.java.simpleName)
    }

    private fun updateWeightAsPerSelectedUnit() {
        HeightWeightUtils.updateWeightValuesAndUIV1(binding.editTextWeight, weight, selectedWeightUnit)
    }
