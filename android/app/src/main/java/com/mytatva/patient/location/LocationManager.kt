...
class LocationManager @Inject constructor(val context: Context) {
    ...
    init {
        locationRequest = LocationRequest.create().setInterval(UPDATE_INTERVAL.toLong())
...
//                                        Timber.i(errorMessage)
    ...
    fun startLocationUpdates(
        ...
        isPermissionGranted { isGranted ->
            if (isGranted) {
                (activity as BaseActivity?)!!.analytics.logEvent(
                    AnalyticsClient.CLICKED_GRANT_LOCATION,
                    Bundle().apply {
                        putString(
...
//        Timber.i("STOP LOCATION UPDATES")
    ...
    private fun requestPermission(): Boolean {
        if (activity != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                //Require only one permission ask fot location
                locationPermissionAPILevel33Greater()

            } else {

                //Require both permission ask for location
                locationPermissionAPILevel33Less()
    ...
    private fun locationPermissionAPILevel33Greater(): Boolean {
    ...
    private fun locationPermissionAPILevel33Less(): Boolean {
        return if (ActivityCompat.checkSelfPermission(
