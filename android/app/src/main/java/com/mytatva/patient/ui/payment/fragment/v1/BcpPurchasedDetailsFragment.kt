...
class BcpPurchasedDetailsFragment : BaseFragment<PaymentFragmentBcpPurchasedDetailsBinding>() {
        ...
        when (view.id) {
            ...
            R.id.buttonRenewNow -> {
                ...
                analytics.logEvent(analytics.RENEW_PLAN, Bundle().apply {
                    ...
                    putString(
                        analytics.PARAM_PLAN_EXPIRY_DATE,
                        bcpPurchasedData?.plan_details?.expiry_date
                    )
                    putString(
                        analytics.PARAM_DAYS_TO_EXPIRE,
                        bcpPurchasedData?.plan_details?.remaining_days
                    )
                }, screenName = AnalyticsScreenNames.BcpPurchasedDetails)
                    ...
                    navigator.loadActivity(
                        ...
                        bundleOf(
                            ...
                            Pair(
                                ...
                                bcpPurchasedData?.plan_details?.plan_type
                            ),
                            /*Pair(
                                Common.BundleKey.PATIENT_PLAN_REL_ID,
                            ...
                            Pair(
                                ...
                                bcpPurchasedData?.plan_details?.enable_rent_buy
                            ),
                        )
                    ).start()
                } else {
    ...
    private fun setProgressOfPlanExpireDetails() = with(binding) {
        ...
        analytics.logEvent(
            ...
            Bundle().apply {
                ...
                putString(
                    analytics.PARAM_PLAN_TYPE,
                    bcpPurchasedData?.plan_details?.plan_type
                )
                putString(
                    analytics.PARAM_CURRENT_PLAN_TYPE,
                    "paid"
                )
                putInt(
        ...
        if (planRemainingDays > 14) {
            ...
            val date =
                DateTimeFormatter.date(planDetails?.expiry_date, DateTimeFormatter.FORMAT_yyyyMMdd)
                    .formatDateToLocalTimeZoneDisplay(DateTimeFormatter.FORMAT_MMMMddyyyy)
            textViewLabelExpireDate.text =
                getString(R.string.bcp_purchase_details_label_expire_date, date)
            textViewLabelExpireDate.setTextColor(
                ContextCompat.getColor(
                    requireContext(),
                    R.color.background_dark_gray
                )
            )
            ...
            if (planRemainingDays < 0) {
                ...
                textViewLabelExpireDate.text =
                    getString(R.string.bcp_purchase_details_label_plan_expire)
                ...
                textViewLabelExpireDate.text = getString(
                    R.string.bcp_purchase_details_label_remaining_days,
                    planRemainingDays.toString()
                )
            }
        ...
        bcpPurchasedData.let { item ->
            if (item?.diagnostic_tests == "Y") {
                ...
                if (totalCount != null) {
                    if (planRemainingDays >= 0) {
                        val remainingCount =
                            totalCount.toInt() - item.duration_details.diagnostic_test_used_count!!.toInt()
                        textViewCount.text = "$remainingCount/$totalCount Left"
                        if (remainingCount == 0) {
                            textViewCount.setTextColor(
                                ContextCompat.getColor(
                                    requireContext(),
                                    R.color.redError
                                )
    ...
    private fun setDaysProgressBarColor(totalDays: String, remainingDays: String, color: Int) =
        with(binding) {
