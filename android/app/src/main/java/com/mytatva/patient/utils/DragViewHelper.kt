...
object DragViewHelper {
    ...
    fun View.cornerAnimationFromCenter(activity: AppCompatActivity, parent: View, animationEnable: Boolean) {
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    if (isLeft) {
                        if (isTop) {
                            ...
                            childView.viewMoveFromToPosition(0f,
                                (screenHeight - childView.height - actionBarHeight).toFloat(),
                                animationEnable)
                        }
                    } else {
                        if (isTop) {
                            childView.viewMoveFromToPosition((screenWidth - childView.width).toFloat(),
                                0f,
                                animationEnable)
                        } else {
                            childView.viewMoveFromToPosition((screenWidth - childView.width).toFloat(),
    ...
    private fun View.viewMoveFromToPosition(
        ...
        if (animationEnable) {
            ...
            val currentX = this.x
            val currentY = this.y
            animatorSet.playTogether(ObjectAnimator.ofFloat(this, "x", currentX, transactionX),
