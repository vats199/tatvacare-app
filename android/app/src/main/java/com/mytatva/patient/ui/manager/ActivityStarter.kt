...
internal constructor(private val context: BaseActivity) {
    ...
    private inner class Builder : ActivityBuilder {
        ...
        override fun <T : BaseFragment<*>> setPage(page: Class<T>): ActivityBuilder {
            if (intent!!.hasExtra(ACTIVITY_FIRST_PAGE)) {
                // if already contains this, while we are passing prev bundle to next screen in Isolated activity
                // remove it
                intent!!.removeExtra(ACTIVITY_FIRST_PAGE)
            }
        ...
        override fun addFlags(flags: Int): ActivityBuilder {
