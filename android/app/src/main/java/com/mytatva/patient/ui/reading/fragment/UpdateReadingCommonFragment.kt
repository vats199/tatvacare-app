...
class UpdateReadingCommonFragment(
            ...
            return try {
                with(binding) {
                    ...
                    when (reading) {
                        ...
                        Readings.CaloriesBurned -> {
                            if (readingValue < ReadingMinMax.MIN_CALORIES_BURNED || readingValue > ReadingMinMax.MAX_CALORIES_BURNED) {
                                throw ApplicationException("Please enter Calories Burned in the range of ${ReadingMinMax.MIN_CALORIES_BURNED} - ${ReadingMinMax.MAX_CALORIES_BURNED}")
                            }
                        }
            ...
            when (reading) {
                ...
                Readings.FVC -> {
                    editTextReading.filters =
                        arrayOf(
                            DecimalDigitsInputFilter(
                                ReadingMinMax.MAX_FVC.toString().length + 3,
                                2
                            )
                        )
                    editTextReading.inputType =
                ...
                Readings.FEV1FVC_RATIO -> {
                    editTextReading.filters =
                        arrayOf(
                            DecimalDigitsInputFilter(
                                ReadingMinMax.MAX_FEV1FVC_RATIO.toString().length + 3,
                                2
                            )
                        )
                    editTextReading.inputType =
                ...
                Readings.AQI -> {
                    editTextReading.filters =
                        arrayOf(
                            DecimalDigitsInputFilter(
                                ReadingMinMax.MAX_AQI.toString().length + 3,
                                2
                            )
                        )
                    editTextReading.inputType =
                ...
                Readings.HUMIDITY -> {
                    editTextReading.filters =
                        arrayOf(
                            DecimalDigitsInputFilter(
                                ReadingMinMax.MAX_HUMIDITY.toString().length + 3,
                                2
                            )
                        )
                    editTextReading.inputType =
                ...
                Readings.TEMPERATURE -> {
                    editTextReading.filters =
                        arrayOf(
                            DecimalDigitsInputFilter(
                                ReadingMinMax.MAX_TEMPERATURE.toString().length + 3,
                                2
                            )
                        )
                    editTextReading.inputType =
                ...
                Readings.CaloriesBurned -> {
                    editTextReading.filters =
                        arrayOf(InputFilter.LengthFilter(ReadingMinMax.MAX_CALORIES_BURNED.toString().length))
                }
    ...
    private fun updatePatientReadings() {
        val apiRequest = ApiRequest().apply {
            ...
            /*if (reading == Readings.CaloriesBurned) {
                val calToPassInAPI = Calendar.getInstance()
                calToPassInAPI.set(Calendar.HOUR_OF_DAY, 0)
                calToPassInAPI.set(Calendar.MINUTE, 0)
                calToPassInAPI.set(Calendar.SECOND, 0)
                reading_datetime = DateTimeFormatter.date(calToPassInAPI.time)
            ...
            when (reading) {
                ...
                Readings.CaloriesBurned -> {
                    val caloriesBurned =
                        binding.editTextReading.text.toString().trim().toFloatOrNull() ?: 0f
                    if (caloriesBurned > 0) {
                        googleFit.writeCaloriesBurned(caloriesBurned, calDateTime)
                    }
                }
