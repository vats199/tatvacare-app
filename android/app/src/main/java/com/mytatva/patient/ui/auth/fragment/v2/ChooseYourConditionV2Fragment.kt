...
class ChooseYourConditionV2Fragment : BaseFragment<AuthFragmentChooseConditionV2Binding>() {
    ...
    private fun updateOtherMedicalConditionUI() {
        binding.groupProvideCondition.isVisible =
            selectedMedicalCondition?.is_other.equals("Yes", true)
        ...
        val apiRequest = ApiRequest().apply {
            session.user.let {
                ...
                medical_condition_group_id =
                    chooseConditionAdapter.list.find { it.isSelected }?.medical_condition_group_id
    ...
    private fun observeLiveData() {
        authViewModel.medicalConditionGroupListLiveData.observe(this,
            onChange = { responseBody ->
                ...
                if (session.user?.medical_condition_group_id.isNullOrBlank()
                        .not() && session.user?.indication_name.isNullOrBlank().not()
                ) {

                    val item =
                        allMedicalConditionList.find { it.medical_condition_group_id == session.user?.medical_condition_group_id }
