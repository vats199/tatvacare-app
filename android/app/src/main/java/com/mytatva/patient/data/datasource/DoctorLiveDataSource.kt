...
class DoctorLiveDataSource @Inject constructor(private val doctorService: DoctorService) :
    ...
    override suspend fun addToCart(
        ...
        return execute { doctorService.addToCart(apiRequest) }.apply {
            //remove coupon code on cart modified
            LabtestCartV1Fragment.updateFlagIfCouponCodeAppliedOnCartModified()
            if (labTestId.isNotBlank()) {
                analytics.logEvent(analytics.TEST_ADDED_TO_CART, Bundle().apply {
                    putString(analytics.PARAM_LAB_TEST_ID, labTestId)
    ...
    override suspend fun removeFromCart(
        ...
        return execute { doctorService.removeFromCart(apiRequest) }.apply {
            //remove coupon code on cart modified
            LabtestCartV1Fragment.updateFlagIfCouponCodeAppliedOnCartModified()
            analytics.logEvent(analytics.TEST_REMOVED_FROM_CART, Bundle().apply {
                putString(analytics.PARAM_LAB_TEST_ID, labTestId)
            }, screenName = screenName)
        }
    }
    ...
    override suspend fun getAppointmentSlots(apiRequest: ApiRequest): DataWrapper<ArrayList<TimeSlotData>> {
    ...
    override suspend fun bookTest(apiRequest: ApiRequest): DataWrapper<BookTestResData> {
        return execute { doctorService.bookTest(apiRequest) }.apply {
            if (this.responseBody?.responseCode == Common.ResponseCode.SUCCESS) {
