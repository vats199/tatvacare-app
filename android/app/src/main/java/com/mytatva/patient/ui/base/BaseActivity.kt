...
BaseActivity : AppCompatActivity(), HasComponent<ActivityComponent>, HasToolbar,
    ...
    private fun showNotificationPermissionRationaleDialog() {
        showAppMessage("Please!! Give permission it's require for notification")
    }
    ...
    override fun openPdfViewer(certificate: String, isLocalFile: Boolean) {
    ...
    fun handleScreenNavigation(deepLink: Uri/*, callbackToDefaultFlow: () -> Unit*/) {
        ...
        when (screenName) {
            ...
            AnalyticsScreenNames.LabtestOrderDetails -> {
    ...
    fun updateReadingsGoals(isCalledOnPermissionApproved: Boolean = false) {
        ...
        if (session.user?.sync_at.isNullOrBlank().not()) {
            try {
                ...
                calForCaloriesConsumed.timeInMillis = DateTimeFormatter.date(
                    session.user!!.sync_at,
                    DateTimeFormatter.FORMAT_yyyyMMdd_HHmmss
                ).date!!.time
                ...
                calForCaloriesConsumed.add(Calendar.DAY_OF_YEAR, -30)
            }
        } else {
            cal.add(Calendar.DAY_OF_YEAR, -30)
            calForCaloriesConsumed.add(Calendar.DAY_OF_YEAR, -30)
