...
class GoogleFit @Inject constructor(val context: Context) {
                        ...
                        for (dataset in bucket.dataSets) {
                            ...
                            for (dataPoint in dataset.dataPoints) {
                                ...
                                try {
                                    ...
                                    cal.timeInMillis =
                                        DateTimeFormatter.date(Date(dataPoint.getTimestamp(
                                            TimeUnit.MILLISECONDS))).date?.time!!
                                    cal.set(Calendar.HOUR_OF_DAY, 0)
                                    cal.set(Calendar.MINUTE, 0)
                                    cal.set(Calendar.SECOND, 0)
    ...
    fun getSleepRecords(
        ...
        val request = SessionReadRequest.Builder()
            ...
            .setTimeInterval(startTime /*tempStartTime.timeInMillis*/,
                endTime /*tempEndTime.timeInMillis*/,
                TimeUnit.MILLISECONDS)
            .build()

        Fitness.getSessionsClient(activity,
            ...
            .addOnSuccessListener { response ->
                for (session in response.sessions) {
                    ...
                    val SLEEP_STAGE_NAMES = arrayOf("Unused", "Awake (during sleep)",
                        ...
                        for (dataset in bucket.dataSets) {
                            ...
                            for (dataPoint in dataset.dataPoints) {
                                ...
                                if (dataPoint.originalDataSource.streamName.equals("user_input")
                                    ...
                                    try {
                                        list.add(dataPoint.getValue(Field.FIELD_DISTANCE).asFloat()
                                            .toDouble())
                                    } catch (e: Exception) {
                                        e.printStackTrace()
                                    }
    ...
    fun writeSleepV2(calStart: Calendar, calEnd: Calendar) {
        activity?.let {
            ...
            Session.Builder()
                .setActivity(FitnessActivities.SLEEP)
                .setStartTime(dataSet.dataPoints.first().getStartTime(TimeUnit.NANOSECONDS),
                    TimeUnit.NANOSECONDS)
                .setEndTime(dataSet.dataPoints.last().getEndTime(TimeUnit.NANOSECONDS),
                    TimeUnit.NANOSECONDS)
                .setName("Sleep")
                .build()
    ...
    fun writeBloodGlucose(
        ...
        activity?.let {
            ...
            dataPoints.add(DataPoint.builder(dataSource)
                .setField(HealthFields.FIELD_BLOOD_GLUCOSE_LEVEL, bgLevelFastMmol)
                .setField(HealthFields.FIELD_TEMPORAL_RELATION_TO_MEAL,
...
fun DataSource.createDataSet(startDateTime: String, vararg sleepPeriods: Pair<Int, Long>): DataSet {
    ...
    for (sleepPeriod in sleepPeriods) {
        ...
        val dataPoint = DataPoint.builder(this)
            ...
            .setTimeInterval(cursorMilliseconds,
