...
//        }
    ...
    override var cart: Cart? = null
        get() {
            if (field == null) {
                val cartJSON = appPreferences.getString(CART_JSON)
                field = gson.fromJson(cartJSON, Cart::class.java)
            }
            return field
        }
        set(value) {
            field = value
            val cartJson = gson.toJson(value)
            if (cartJson != null)
                appPreferences.putString(CART_JSON, cartJson)
        }
