...
@Singleton
class AnalyticsClient @Inject constructor(private val session: Session, private val context: Context) {
...
//    val SHOW_BOTTOM_SHEET = "SHOW_BOTTOM_SHEET"
    ...
    fun setScreenName(screenName: String) {
        ...
        firebaseAnalytics?.logEvent(
            FirebaseAnalytics.Event.SCREEN_VIEW, bundleOf(Pair(FirebaseAnalytics.Param.SCREEN_NAME, screenName))
        )
    ...
    fun login(user: com.mytatva.patient.data.pojo.User) {
        user.patient_id?.let {
            ...
            weUser?.setAttribute("indication", user.medical_condition_name?.firstOrNull()?.medical_condition_name)
            weUser?.setAttribute("dr_name", user.patient_link_doctor_details?.firstOrNull()?.name)
            weUser?.setAttribute("dr_phone", user.patient_link_doctor_details?.firstOrNull()?.contact_no)
            ...
            weUser?.setAttribute("patientGender", if (user.gender == "M") "Male" else if (user.gender == "F") "Female" else "")
            ...
            ApxorSDK.setUserCustomInfo(Attributes().apply {
                putAttribute("indication", user.medical_condition_name?.firstOrNull()?.medical_condition_name)
                putAttribute("dr_name", user.patient_link_doctor_details?.firstOrNull()?.name)
                putAttribute("dr_phone", user.patient_link_doctor_details?.firstOrNull()?.contact_no)
            ...
            Freshchat.getInstance(getApplicationContext()).identifyUser(user.patient_id, user.restore_id)
            ...
            Freshchat.getInstance(getApplicationContext()).setPushRegistrationToken(session.deviceId)
